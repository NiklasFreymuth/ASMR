...
def build_gaussian(mean: np.array,
                   initial_covariance: np.array,
                   rotation_angle: float) -> MultivariateNormal:
    """

    Args:
        mean: Mean position of the Gaussian
        initial_covariance: The initial covariance uncorrelated covariance as a diagonal matrix
        rotation_angle: A rotation angle in [0, 2pi]

    Returns: A pytorch distributions MultivariateNormal distribution with the given mean and rotated covariance

    """
    rotation_matrix = np.matrix([[np.cos(rotation_angle), -np.sin(rotation_angle)],
                                 [np.sin(rotation_angle), np.cos(rotation_angle)]])
    covariance = rotation_matrix @ initial_covariance @ rotation_matrix.T

    gaussian = MultivariateNormal(loc=torch.tensor(mean), covariance_matrix=torch.tensor(covariance))
    return gaussian
