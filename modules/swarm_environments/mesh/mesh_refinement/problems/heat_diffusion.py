...
class HeatDiffusion(AbstractFiniteElementProblem):
    ...
    def __init__(self, *,
        ...
        self._diffusivity_range = [diffusion_config.get("lower_diffusivity"),
    ...
    def _set_pde(self) -> None:
        ...
        if self._fixed_diffusion:
            ...
            self.diffusivity = self._random_state.uniform(low=self._diffusivity_range[0],
                                                          high=self._diffusivity_range[1],
                                                          size=1).item()
    ...
    def _get_error_estimate_from_indicator(self, basis: Basis, solution: np.array) -> np.array:
        """
        """
        # Only used in Poisson, (unused)
        raise NotImplementedError("Error estimate not implemented for Heat Source Diffusion equation")
