...
from modules.swarm_environments.mesh.mesh_refinement.problems.abstract_finite_element_problem import AbstractFiniteElementProblem
...
class LinearElasticity(AbstractFiniteElementProblem):
    def __init__(self, *,
        ...
        self._stress_type = linear_elasticity_config.get("stress_type")  # "von_mises" or "euclidean"
    ...
    def _calculate_solution(self, basis: Basis, cache: bool) -> np.array:
        ...
        if self._stress_type == "von_mises":  # calculate von Mises stress
            s = {
            ...
            stress_norm = np.sqrt(.5 * ((s[0, 0] - s[1, 1]) ** 2 +
                                        ...
                                        (s[2, 2] - s[0, 0]) ** 2 +
                                        6. * s[0, 1] ** 2))
        elif self._stress_type == "euclidean":
            stress_norm = np.sqrt(sigma[sgb.nodal_dofs[0]] ** 2 + sigma[sgb.nodal_dofs[1]] ** 2 +
                                  sigma[sgb.nodal_dofs[2]] ** 2 + sigma[sgb.nodal_dofs[3]] ** 2)

        else:
            raise ValueError(f"Unknown stress type {self._stress_type}")
    ...
    def _get_error_estimate_from_indicator(self, basis: Basis, solution: np.array) -> np.ndarray:
        raise NotImplementedError("Error estimate not implemented for LinearElastics equation")
    ...
    def element_features(self, basis: Basis, element_feature_names: List[str]) -> Optional[np.array]:
        ...
        """
        features = []
        num_elements = basis.mesh.nelements
        if "x_displacement" in element_feature_names:
            features.append(np.repeat(np.array(self._boundary_x_displacement), repeats=num_elements, axis=0))
        if "y_displacement" in element_feature_names:
            features.append(np.repeat(np.array(self._boundary_y_displacement), repeats=num_elements, axis=0))
